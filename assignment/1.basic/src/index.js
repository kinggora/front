...
const completeFolderTree = function (folderList) {
    for (let i = 0; i < folderList.length; i++) {
        folderList[i].addEventListener('click', (event) => {
            ...
            if (toggle) {
                if(toggle.classList.contains('isOpen')) { // 폴더 구조 닫기
                    toggle.classList.remove('isOpen'); // 상위 토글

                    const toggles = event.target.parentNode.querySelectorAll('.toggle'); // 하위 토글들
                    toggles.forEach((n) => {
                         n.classList.remove('isOpen');
                    });

                    const lowerFolder = event.target.parentNode.querySelectorAll('.lower-folder'); // 모든 하위 폴더
                    lowerFolder.forEach((n) => { 
                         n.style.display = 'none';
                    });
                } else { // 폴더 구조 열기
                    toggle.classList.add('isOpen'); // 상위 토글
                    const lowerFolder = event.target.parentNode.querySelector('.lower-folder'); // 하위 폴더
